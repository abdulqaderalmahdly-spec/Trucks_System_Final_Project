...
# إعدادات قاعدة البيانات الديناميكية
DATABASE_URL = os.environ.get('DATABASE_URL')
if DATABASE_URL:
    # في بيئة الإنتاج (Render)، استخدم PostgreSQL
    # Render تقوم تلقائياً بتغيير 'postgres://' إلى 'postgresql://'
    app.config['SQLALCHEMY_DATABASE_URI'] = DATABASE_URL.replace('postgres://', 'postgresql://', 1)
else:
    # في البيئة المحلية، استخدم SQLite
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///trucks_system.db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['JSON_ARABIC_SUPPORT'] = True
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'your-default-secret-key')  # يجب تغييره في الإنتاج
